...
const SearchInput = <T extends FieldValues>({
  ...
  return (
    <div className={cn('relative', classContent)}>
      <FormInput name={name} form={form} placeholder={placeholder}
                 autoFocus={autoFocus} type={type} />
      <div
        className={cn('absolute bg-slate-50 rounded-sm z-20 p-2 transform -translate-y-2 w-full  ', listAddress === null && 'hidden')}>
        {
          listAddress && listAddress?.length > 0 &&
          listAddress?.map((address, index) => (
            <div className={cn('text-sm py-1 px-2 hover:bg-slate-100 rounded')} key={index}
                 onClick={() => onChoose(address['description'], address['place_id'])}>{address['description']}</div>))
